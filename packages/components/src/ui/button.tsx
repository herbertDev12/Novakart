...
function Button({
  ...
  return (
    <Comp
      ...
      {...props}
    >
      <LoadingIcon loading={loading} StartIcon={StartIcon} />
      {children}
      {EndIcon && !loading && <EndIcon className="h-4 w-4" />}
    </Comp>
  )
}

const LoadingIcon = ({ loading, StartIcon }: { loading?: boolean; StartIcon?: LucideIcon }) => {
  if (loading) return <LoaderCircle className="animate-spin w-4 h-4" />
  return StartIcon && <StartIcon className="h-4 w-4" />
}
